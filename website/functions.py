...
def generate_funny_nickname():
    fake = Faker()
    prefixes = [
        "Пухлый", "Лучший", "Низкий", "Смелый", "Быстрый", "Ручной",
        "Длинный", "Влажный", "Чрезвычаен", "Дерзкий",
        "Шустрый", "Лютый", "Важный", "Экстренный"
    ]
    prefix = random.choice(prefixes)
    nr = random.randint(1, 99)
    suffixes = [
        "Мальчик", "Банан", "Монстр", "Донатер", "Легенда", "Элита",
        "Босс", "Красвчик", "Читер"
    ]
    suffix = random.choice(suffixes)
    # Combine to form the nickname
    nickname = f"{prefix}{suffix}{nr}"
