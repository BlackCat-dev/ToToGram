...
def handle_new_message(data):
    if current_user.is_authenticated:
        ...
        if message.startswith("/"):
            ...
            db_private_message = PrivateMessage(data=message, sender_id=user.id, recipient_id=recipient_id) # type: ignore
...
def dm_view(recipient_id):
    ...
    if request.method == 'POST':
        ...
        if message_content:
            send_private_message(current_user, recipient, message_content) # type: ignore

    private_messages = get_private_messages(current_user, recipient) # type: ignore
